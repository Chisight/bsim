...
const NodeRenderer = {
    /**
     * [AUDIT: v1.23.78 | SEC_ARCH_LEAD] - Entry trace for node DOM instantiation.
     * @ARCH: UI_RENDERING
     * @IO: DOM_FACTORY
    ...
    renderNode(node) {
        ...
        if (node.type.startsWith('IN-')) {
            for (let i = 0; i < bits; i++) {
                ...
                // [AUDIT: v1.23.78 | SEC_ARCH_LEAD] - UI Scaling: Render single-digit pin indices for buses to prevent layout clipping.
                ...
                // [AUDIT: v1.23.78 | SEC_ARCH_LEAD] - Isolated bit-dot indicators to opposite bounds to prevent port interaction overlap.
            ...
            if (node.type === 'JUNCTION') portsHtml = `<div class="port input output" data-port="j" style="top:50%;left:50%;transform:translate(-50%,-50%)" onmousedown="if(!Sim.wiring.active) return; event.stopPropagation(); Sim.handlePortInteraction(event, '${node.id}', 'j')"></div>`;
                ...
                if (chipDef) {
                    // [AUDIT: v1.23.78 | SEC_ARCH_LEAD] - Apply secondary X-axis sorting to prevent creation-order drift for horizontal components.
                    ...
                    ins.forEach((p) => {
                        ...
                        for (let i = 0; i < bits; i++) {
                            ...
                            const portId = `in${cIn}`;
                            // [AUDIT: v1.23.78 | SEC_ARCH_LEAD] - UI Scaling: Render single-digit pin indices for custom macro buses.
        ...
        // [AUDIT: v1.23.78 | SEC_ARCH_LEAD] - EXIT_TRACE: Node rendered and appended to DOM: ${node.id} (${node.type}).
    }
};
