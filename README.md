# bSim v1.25.21 - Professional Modular Logic Simulator

> [!IMPORTANT]
> **Architectural Status v1.25.21**: This version represents the stable hardened baseline. Significant engine hardening has been implemented to resolve memory synchronization faults, register core architectural primitives like Constant Ground (0), and optimize mobile UX via dynamic viewport scaling (dvh).
...
- **Hybrid Parity**: (v1.25.21) Full opcode dispatchers including the native **Opcode 9 (CONST_0)** for grounding and centralized Wasm memory evaluation.
...
(v1.25.21) Optimized for productivity across all form factors.
...
- **Dynamic Viewport**: Implemented `100dvh` to prevent mobile browser chrome occlusion of the UI footer.
...
```text
browser-sim/
├── index.html          # Main entry point (v1.25.21 Architecture)
